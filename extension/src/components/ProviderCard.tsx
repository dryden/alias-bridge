...
export function ProviderCard({ providerId, isPro, onConfigChange }: ProviderCardProps) {
    ...
    useEffect(() => {
        if (providerId === 'simplelogin') {
            ...
            // Ensure the config reflects this requirement
    ...
    const checkDomainCatchAllStatus = async (domain: string) => {
        ...
        try {
            ...
            if (details) {
                ...
                // Always lock the checkbox for Addy as the behavior is determined by domain status
                setIsWaitServerConfirmationDisabled(true);
                            ...
                            <div className="flex items-start gap-3">
                                <div className="relative mt-1">
                                    <Checkbox
                                        ...
                                        checked={providerId === 'simplelogin' ? true : (config.waitServerConfirmation === true)}
                                        ...
                                        {providerId === 'addy' ? (
                                            <>
                                                {catchAllStatus === 'disabled' && (
                                                    <p>
                                                        <span className="text-slate-300 font-semibold">Catch-all Disabled Domain:</span> The selected domain has the catch-all feature disabled. This app cannot customize aliases via API. The system will call the Addy API to generate a random email and paste it after you press the button.
                                                    </p>
                                                )}
                                                {catchAllStatus === 'enabled' && (
                                                    <p>
                                                        <span className="text-slate-300 font-semibold">Catch-all Enabled Domain:</span> The selected domain has the catch-all feature enabled. Official restrictions prevent pre-creating aliases in the Addy list via API. Addy will automatically create the alias when the first email is received.
                                                    </p>
                                                )}
                                                {!catchAllStatus && (
